function CovidInfo({ selection, arr }) {
  const obj = arr[0];
  return (
    <div>
      {selection.length ? (
        ...
        <div className="info">
          <div className="info__item population">
            <span className="info__item-title">Population</span>
            <span className="info__item-elem">
              {obj.population.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')}
            </span>
          </div>
          <div className="info__item population">
            <span className="info__item-title">Life expectancy</span>
            <span className="info__item-elem">{obj.life_expectancy}</span>
          </div>
          <div className="info__item confirmed">
            <span className="info__item-title">Confirmed</span>
            <span className="info__item-elem">
              {obj.confirmed.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')}
            </span>
          </div>
          <div className="info__item recovered">
            <span className="info__item-title">Recovered</span>
            <span className="info__item-elem">
              {obj.recovered.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')}
            </span>
          </div>
          <div className="info__item confirmed">
            <span className="info__item-title">
            ...
            <span className="info__item-elem">{(obj.confirmed / obj.sq_km_area).toFixed(1)}</span>
          </div>
          <div className="info__item confirmed">
            <span className="info__item-title">Infections per thousand</span>
            <span className="info__item-elem">
              {((obj.confirmed / obj.population) * 1000).toFixed(1)}
            </span>
          </div>
          <div className="info__item deaths">
            <span className="info__item-title">Deaths</span>
            <span className="info__item-elem">
              {obj.deaths.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')}
            </span>
          </div>
          <div className="info__item deaths">
            <span className="info__item-title">Mortality rate, %</span>
            <span className="info__item-elem">
              {((obj.deaths / obj.confirmed) * 100).toFixed(1)}
            </span>
          </div>
        </div>
      )}
